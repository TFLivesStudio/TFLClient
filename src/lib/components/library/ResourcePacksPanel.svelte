<script lang="ts">
	...
	async function refreshInstalled() {
		installed = await getInstanceResourcepacks(instance.name);
	}
	...
	async function runSearch(q: string) {
		...
		error = null;
		try {
			const hits = await searchResourcepacks(q, instance.mc_version);
	...
	async function handleInstall(projectId: string) {
		...
		error = null;
		try {
			await installResourcepack(instance.name, projectId, instance.mc_version);
	...
	async function handleRemove(filename: string) {
		try {
			await removeResourcepack(instance.name, filename);
			await refreshInstalled();
		} catch (e) {
			error = String(e);
		}
...
<div class="resourcepacks-panel">
	...
	<div class="search-row">
		...
		{/if}
		<input type="text" bind:value={query} placeholder="Buscar resource packs en Modrinth…" />
	</div>
